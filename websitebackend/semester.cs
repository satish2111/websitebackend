...
namespace websitebackend
{
    public partial class semester : Form
    {
        ...
        string usernamefind;
        int? pkidsemester;
        public semester(string valueid)
        {
            InitializeComponent();
        ...
        private void btnadd_Click(object sender, EventArgs e)
        {
            ...
            string StrQuery = "SELECT MAX(pkidsemester) FROM dbo.semester";
        ...
        public void datagridload()
        {
            ...
            {
                type = "SELECTONE";
                dic.Add("@pkidsemester", txtsearch.Text);
            }
            else
            {
                type = "SELECTALL";
            ...
            if (objDB.objExecuteQuery("Sp_admin_semester_select", clsSqlHelper.QueryExcution.storeProcedure, dic))
            {
        ...
        public void idsemester()
        {
            clsSqlHelper objDB = new clsSqlHelper(ConfigurationManager.ConnectionStrings["CN"].ConnectionString);
            Dictionary<string, object> dic = new Dictionary<string, object>();
            dic.Add("@semester", usernamefind);
            string qu = "SELECT pkidsemester FROM dbo.semester WHERE semester=@semester";
            objDB.objExecuteQuery(qu, clsSqlHelper.QueryExcution.ExecuteReader, dic);
            if (objDB.dtrData.HasRows && objDB.dtrData.Read() && !objDB.dtrData.IsDBNull(0))
            {
                pkidsemester = objDB.dtrData.GetInt32(0);
            }
        }
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            if (txtcourse.Text.Trim().Length != 0 && txtcategory.Text.Trim().Length != 0)
            {
                clsSqlHelper objDB = new clsSqlHelper(ConfigurationManager.ConnectionStrings["CN"].ConnectionString);
                Dictionary<string, object> dic = new Dictionary<string, object>();
                dic.Add("@fkcourse", txtcourse.SelectedValue);
                dic.Add("@fkidcategory", txtcategory.SelectedValue);
                dic.Add("@semester", txtsemester.Text);
                ...
                else if (btnsave.Text == "Update")
                {
                    ...
                    dic.Add("@pkidsemester", values.Text);
                }
                if (objDB.objExecuteQuery("Sp_admin_semester_insert_update", clsSqlHelper.QueryExcution.storeProcedure, dic))
                {
                    DataTable dt = new DataTable();
                    dt = objDB.objDataset.Tables[0];
                    ...
                    else { AutoClosingMessage.AutoClosingMessageBox.Show("Data Already Saved  " + Environment.NewLine + " Semester ID:-" + dt.Rows[0][0].ToString() + "", "Message", 5000); }
                    datagridload();
                    ClearTextBoxes(this);
                    savecancel();
                }
        ...
        private void txtcategory_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            else if (btnsave.Text == "Save" && txtcategory.Text.Trim().Length != 0 && txtcategory.SelectedIndex != 0)
            {
                txtsemester.Focus();
            }
        }
        ...
        {
            if (txtcategory.Text.Trim().Length != 0 && e.KeyCode == Keys.F1)
            {
                categoryidname();
